...
SaveJoinLogs = Instance.new("TextButton")
...
SaveJoinLogs.Name = "SaveJoinLogs"
SaveJoinLogs.Parent = join
SaveJoinLogs.BackgroundColor3 = Color3.new(0.180392, 0.180392, 0.184314)
SaveJoinLogs.BorderSizePixel = 0
SaveJoinLogs.Position = UDim2.new(0, 258, 0, 220)
SaveJoinLogs.Size = UDim2.new(0, 75, 0, 20)
SaveJoinLogs.ZIndex = 10
SaveJoinLogs.Font = Enum.Font.SourceSans
SaveJoinLogs.FontSize = Enum.FontSize.Size14
SaveJoinLogs.Text = "Save To .txt"
SaveJoinLogs.TextColor3 = Color3.new(1, 1, 1)
table.insert(shade2,SaveJoinLogs)
table.insert(text1,SaveJoinLogs)
...
rcdEnabled = function()
	local success, isEnabled = pcall(function()
		local value, wasHidden = gethidden(workspace, "RejectCharacterDeletions") ~= Enum.RejectCharacterDeletions.Disabled
		return value
	end)
	return success and isEnabled or false
end
...
function writefileExploit()
	return vtype(writefile, "function")
end

function readfileExploit()
	return vtype(readfile, "function")
end

function isNumber(t)
	return not not tonumber(t)
end

function getPos(args, vector3)
	...
	for _, v in ipairs(args) do
		...
		table.insert(pos, num)
	end
	return vector3 and Vector3.new(unpack(pos)) or CFrame.new(unpack(pos))
end
...
function tools(plr)
	...
	return not not (plr.Backpack:FindFirstChildWhichIsA("BackpackItem") or (char and char:FindFirstChildWhichIsA("BackpackItem")))
end
...
On.BackgroundTransparency = StayOpen == false and 1 or 0
...
SaveChatlogs.Activated:Connect(function()
	if writefileExploit() then
		...
		notify('Chat Logs','Your exploit does not support writefile. You cannot save chat logs.')
	end
end)

SaveJoinLogs.Activated:Connect(function()
	if writefileExploit() then
		if scroll_3:FindFirstChildOfClass("Frame") then
			notify("Loading",'Hold on a sec')
			local placeName = CleanFileName(MarketplaceService:GetProductInfo(PlaceId).Name)
			local writelogs = '-- Infinite Yield join logs for '..placeName

			for _, entry in pairs(scroll_3:GetChildren()) do
			...
			local function nameFile()
				...
				if file then
					...
					writefileCooldown(placeName..' Join Logs ('..fileext..').txt', writelogs)
				end
			end
			nameFile()
			notify('Join Logs','Saved join logs to the workspace folder within your exploit folder.')
		end
	else
		notify('Join Logs','Your exploit does not support writefile. You cannot save join logs.')
	end
end)
...
Players.PlayerRemoving:Connect(function(player)
	...
	if player == Players.LocalPlayer then
		...
		writefile('PreviousServer.json', HttpService:JSONEncode(serverInfo), true)
	end
...
CMDs[#CMDs + 1] = {NAME = 'lastserver / ls', DESC = 'Teleports you to the last server you were in'}
...
CMDs[#CMDs + 1] = {NAME = 'nofavorite / nopromptfav', DESC = 'Prevents the game from prompting you to favorite it (Detectable in some games)'}
...
CMDs[#CMDs + 1] = {NAME = 'setfpscap / fpscap [fps]', DESC = 'Limits your max FPS to the provided number'}
...
CMDs[#CMDs + 1] = {NAME = 'chams [player]', DESC = 'ESP but without text in the way. (Defaults to all players if none is provided)'}
CMDs[#CMDs + 1] = {NAME = 'nochams / unchams [player]', DESC = 'Removes chams'}
...
CMDs[#CMDs + 1] = {NAME = 'copyname / copyuser [player]', DESC = 'Copies a player\'s full username to your clipboard'}
CMDs[#CMDs + 1] = {NAME = 'copydisplay / display [player]', DESC = 'Copies a player\'s display name to your clipboard'}
CMDs[#CMDs + 1] = {NAME = 'copyprofile / profile [player]', DESC = 'Copies a player\'s profile link to your clipboard'}
...
CMDs[#CMDs + 1] = {NAME = 'skidfling / sfling', DESC = 'Basically targeted fling, credit to AnthonyIsntHere'}
...
CMDs[#CMDs + 1] = {NAME = 'invisfling', DESC = 'Enables invisible fling (partially patched, no longer invisible)'}
...
CMDs[#CMDs + 1] = {NAME = 'permadeath', DESC = 'Prevents you from respawning after death in most cases'}
...
CMDs[#CMDs + 1] = {NAME = 'breakvelocity / bv', DESC = 'Sets your characters velocity to 0'}
...
CMDs[#CMDs + 1] = {NAME = 'notifytools / ntools [player]', DESC = 'Notifies you a list of the player\'s current tools'}
...
CMDs[#CMDs + 1] = {NAME = 'unlistento', DESC = 'Disables listento / listentopos'}
CMDs[#CMDs + 1] = {NAME = 'listentopos [X Y Z]', DESC = 'Listens to the area around a certain position'}
...
function refresh(plr)
	...
	local pos1 = workspace.CurrentCamera and workspace.CurrentCamera.CFrame

	plr.CharacterAdded:Once(function(char)
		if pos then char:WaitForChild("HumanoidRootPart").CFrame = pos end
		task.wait(.1)
		if workspace.CurrentCamera and pos1 then workspace.CurrentCamera.CFrame = pos1 end
		refreshCmd = false
	end)
	respawn(plr)
end
...
Players.LocalPlayer.CharacterAdded:Connect(function(char)
	...
	if not Clip then execCmd('clip') end
		if spawnpoint and spawnpos and not refreshCmd then
			task.wait(spDelay)
			char:WaitForChild("HumanoidRootPart").CFrame = spawnpos
		end
	onDied()
end)
onDied()
	...
	task.spawn(function()
		...
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not COREGUI:FindFirstChild(plr.Name..'_ESP') then
			...
			if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				espLoopFunc = RunService.PreRender:Connect(espLoop)
			end
		end
	end)
end
	...
	task.spawn(function()
		...
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not COREGUI:FindFirstChild(plr.Name..'_LC') then
			...
			if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				lcLoopFunc = RunService.PreRender:Connect(lcLoop)
			end
		end
	end)
end
...
function addPlugin(name, silent)
	if name:lower() == 'plugin file name' or name:lower() == 'iy_fe.iy' then
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/rockythef0x/rockyyield/master/source'))()")
	end
end)
...
addcmd('wait', {}, function(args, speaker)
	if tonumber(args[1]) then task.wait(args[1]) end
end)

addcmd('gametp',{'gameteleport'},function(args, speaker)
	if tonumber(args[1]) then TeleportService:Teleport(args[1]) end
end)
...
addcmd('lastserver', {'ls'}, function(args, speaker)
	if isfile('PreviousServer.json') then
		local lastServer = HttpService:JSONDecode(readfile('PreviousServer.json'))
		TeleportService:TeleportToPlaceInstance(lastServer.placeId, lastServer.jobId, speaker)
	else
		notify('No Server', 'There is no last server saved')
	end
end)
...
addcmd('togglenoclip',{},function(args, speaker)
	execCmd((Clip and 'noclip' or 'clip')..(args[1] == 'nonotify' and ' nonotify' or ''))
end)
...
function unmobilefly(speaker)
	...
	if root then
		local vel, gyro = root:FindFirstChild(velocityHandlerName), root:FindFirstChild(gyroHandlerName)
...
addcmd('qefly',{'flyqe'},function(args, speaker)
	...
	if not args[1] then QEfly = not QEfly; return end
	if arg then QEfly = (arg == nil and not QEfly) or (arg == 'true' and true) or (arg == 'false' and false) end
end)

addcmd('togglefly',{},function(args, speaker)
	execCmd(FLYING and 'unfly' or 'fly')
end)
...
addcmd('cframeflyspeed',{'cflyspeed'},function(args, speaker)
	...
	if speed then CFspeed = speed end
end)

addcmd('togglecfly',{},function(args, speaker)
	execCmd(CFloop and 'uncframefly' or 'cframefly')
end)
...
addcmd('float', {'platform'},function(args, speaker)
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			if args[1] ~= 'nonotify' then notify('Float','Float Enabled (Q = down & E = up)') end
...
addcmd('unfloat',{'nofloat','unplatform','noplatform'},function(args, speaker)
	Floating = false
	local pchar = speaker.Character
	if args[1] ~= 'nonotify' then notify('Float','Float Disabled') end
...
addcmd('togglefloat',{},function(args, speaker)
	execCmd((Floating and 'unfloat' or 'float')..(args[1] == 'nonotify' and ' nonotify' or ''))
end)
...
addcmd('toggleswim',{},function(args, speaker)
	execCmd(swimming and 'unswim' or 'swim')
end)
...
addcmd('allowrejoin',{'allowrj'},function(args, speaker)
	...
	if not args[1] then allow_rj = not allow_rj; return end
	if arg then allow_rj = (arg == 'true' and true) or (arg == 'false' and false) end
	notify('Client AntiTP', 'Allow rejoin set to '..tostring(allow_rj))
end)
...
addcmd('volume',{'vol'},function(args, speaker)
	local level = (tonumber(args[1]) or 10)/10
...
addcmd('chams',{},function(args, speaker)
	if not ESPenabled then
		CHMSenabled = true
		for i,v in pairs((args[1] and getPlayer(args[1], speaker)) or Players:GetPlayers()) do
			if v.Name ~= speaker.Name then
				CHMS(args[1] and Players[v] or v)
			end
		end
	else
...
addcmd('view',{'spectate'},function(args, speaker)
	...
	for i,v in pairs(players) do
		...
		local function viewDiedFunc()
			if not viewing.Character then viewing.CharacterAdded:Wait() end
			workspace.CurrentCamera.CameraSubject = viewing.Character
		end
...
addcmd('enableshiftlock',{'enablesl','shiftlock'},function(args, speaker)
	local function noSl() speaker.DevEnableMouseLock = true end
	speaker:GetPropertyChangedSignal("DevEnableMouseLock"):Connect(noSl)
	noSl()
...
addcmd('noclipcam', {'nccam'}, function(args, speaker)
	...
	if not success then speaker.DevCameraOcclusionMode = Enum.DevCameraOcclusionMode.Invisicam end
end)

addcmd('maxzoom',{},function(args, speaker)
	if tonumber(args[1]) then
		speaker.CameraMaxZoomDistance = args[1] or StarterPlayer.CameraMaxZoomDistance
	end
end)

addcmd('minzoom',{},function(args, speaker)
	if tonumber(args[1]) then
		speaker.CameraMinZoomDistance = args[1] or StarterPlayer.CameraMinZoomDistance
	end
end)
...
addcmd('antiafk',{'antiidle'},function(args, speaker)
	...
	if args[1] ~= 'nonotify' then notify('Anti Idle','Anti idle is enabled') end
end)
...
addcmd("noprompts", {"nopurchaseprompts"}, function(args, speaker)
	COREGUI:WaitForChild("PurchasePromptApp").Enabled = false
end)

addcmd("showprompts", {"showpurchaseprompts"}, function(args, speaker)
	COREGUI:WaitForChild("PurchasePromptApp").Enabled = true
end)

addcmd('nofavorite', {'nopromptfav'}, function(args, speaker)
	COREGUI.AvatarEditorPromptsApp.Children.Enabled = false

	if args[1] ~= 'nonotify' then
		notify('Blocked', 'Blocked game from sending favorite requests')
	end
end)
...
addcmd('copydisplay', {'display'}, function(args, speaker)
	for _, v in ipairs(getPlayer(args[1], speaker)) do
		toClipboard(tostring(Players[v].DisplayName))
	end
end)
...
addcmd('setcreatorid',{'setcreator'},function(args, speaker)
	local targetId = game.CreatorType == Enum.CreatorType.User and game.CreatorId or GroupService:GetGroupInfoAsync(game.CreatorId).Owner.Id
	local targetName = Players:GetNameFromUserIdAsync(targetId)

	StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.PlayerList, false)
	local success, err = pcall(function() speaker.UserId = targetId end)

	if not success then
		Players:SetLocalPlayerInfo(targetId, speaker.Name, speaker.DisplayName, Enum.MembershipType.None, false)
	end
	task.wait(.5)
	StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.PlayerList, true)

	notify('Set ID','Set UserId to '..targetId)
end)

addcmd('appearanceid',{'aid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do 
		notify('Appearance ID', tostring(Players[v].CharacterAppearanceId))
	end
end)
...
addcmd('copyappearanceid',{'caid'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		toClipboard(Players[v].CharacterAppearanceId)
	end
end)
...
addcmd("orbit", {}, function(args, speaker)
	...
	if target and target.Character and getRoot(target.Character) and root and humanoid then
		...
		orbit2 = RunService.PreRender:Connect(function()
			pcall(function()
...
addcmd("strengthen", {}, function(args, speaker)
	...
	for _, child in pairs(speaker.Character:GetDescendants()) do
		if child.ClassName == "Part" then
			child.CustomPhysicalProperties = PhysicalProperties.new((args[1] or 100), 0.3, 0.5)
		end
	end
end)
...
addcmd("weaken", {}, function(args, speaker)
	...
	for _, child in pairs(speaker.Character:GetDescendants()) do
		if child.ClassName == "Part" then
			child.CustomPhysicalProperties = PhysicalProperties.new((-args[1] or 0), 0.3, 0.5)
		end
	end
end)
...
addcmd("breakvelocity", {"bv"}, function(args, speaker)
...
addcmd('nolimbs',{'rlimbs'},function(args, speaker)
	if rcdEnabled() then notify('RejectCharacterDeletions', 'Due to RejectCharacterDeletions being enabled in this game, this command will only be visible to you', 3) end
	if r15(speaker) then
...
addcmd("nosit", {"antisit"}, function(args, speaker)
	execCmd("unnosit")
	local function noSitFunc(char)
		char:WaitForChild("Humanoid"):SetStateEnabled(Enum.HumanoidStateType.Seated, false)
	end
	noSitFunc(speaker.Character)
	noSitConnection = speaker.CharacterAdded:Connect(noSitFunc)
end)

addcmd("unnosit", {"unantisit"}, function(args, speaker)
...
addcmd('autojump',{'ajump'},function(args, speaker)
	...
	autoJump()
	HumanModCons.ajLoop = (HumanModCons.ajLoop and HumanModCons.ajLoop:Disconnect() and false) or RunService.PreRender:Connect(autoJump)
...
addcmd('edgejump',{'ejump'},function(args, speaker)
	...
	edgejump()
	HumanModCons.ejLoop = (HumanModCons.ejLoop and HumanModCons.ejLoop:Disconnect() and false) or RunService.PreRender:Connect(edgejump)
...
addcmd('animspeed',{},function(args, speaker)
	...
	for i,v in next, Animator:GetPlayingAnimationTracks() do
...
addcmd("copyanimationid", {"copyanimid", "copyemoteid"}, function(args, speaker)
	local plr = Players[getPlayer(args[1], speaker)[1]]
	if not plr then return end

	local found = "Animations Copied"
	for _, v in pairs(plr.Character:FindFirstChildOfClass("Humanoid"):GetPlayingAnimationTracks()) do
...
addcmd('tpposition',{'tppos'},function(args, speaker)
	...
	if char and getRoot(char) then
		getRoot(char).CFrame = getPos(args)
	end
end)
...
addcmd('tweentpposition',{'ttppos'},function(args, speaker)
	local char = speaker.Character
	if char and getRoot(char) then
		TweenService:Create(getRoot(speaker.Character), TweenInfo.new(tweenSpeed, Enum.EasingStyle.Linear), {CFrame = getPos(args)}):Play()
	end
end)

addcmd('offset',{},function(args, speaker)
	if speaker.Character then
		speaker.Character:TranslateBy(getPos(args, true))
	end
end)
...
addcmd('getposition',{'getpos','notifypos','notifyposition'},function(args, speaker)
	...
	for i,v in pairs(players)do
		...
		notify('Current Position', tostring(pos:Floor()))
	end
end)
...
addcmd('walktopos',{'walktoposition'},function(args, speaker)
	...
	humanoid.WalkToPoint = getPos(args, true)
end)

addcmd('speed',{'ws','walkspeed'},function(args, speaker)
	local speed = args[2] or args[1]
	if speed == nil or tonumber(speed) then
		speaker.Character:FindFirstChildOfClass('Humanoid').WalkSpeed = speed or 16
	end
end)
...
addcmd('loopjumppower',{'loopjp','loopjpower'},function(args, speaker)
	...
	if isNumber(jpower) then
		...
		local function JumpPowerChange()
			if Char and Human then
				Human[Human.UseJumpPower and "JumpPower" or "JumpHeight"] = jpower
			end
		end
		JumpPowerChange()
...
addcmd('unloopjumppower',{'unloopjp','unloopjpower'},function(args, speaker)
	...
	if Char and Human then
		Human[Human.UseJumpPower and "JumpPower" or "JumpHeight"] = args[1]
	end
end)

addcmd('tools',{'gears'},function(args, speaker)
	for _, service in ipairs({Lighting, ReplicatedStorage}) do
		for _, v in ipairs(service:GetDescendants()) do
			if v:IsA("BackpackItem") then
				local clonedTool = v:Clone()
				clonedTool.Parent = speaker.Backpack
			end
		end
	end
...
addcmd('notifytools', {'ntools'}, function(args, speaker)
	local plr = Players[getPlayer(args[1], speaker)[1]]
	local tools = {}

	for _, v in ipairs(plr.Backpack:GetChildren()) do
		if v:IsA("BackpackItem") then table.insert(tools, v.Name) end
	end
	...
	notify(plr.Name.."'s Tools", #tools > 0 and table.concat(tools, ", ") or "Player has no tools!")
end)
...
addcmd('remotespy',{'rspy'},function(args, speaker)
	...
	if hookfunction and getrawmetatable then
		...
		notify("Incompatible Exploit", "Your exploit does not support this command (missing hookfunction/getrawmetatable)")
	end
end)
...
addcmd('loopgoto',{},function(args, speaker)
	...
	for i,v in pairs(players) do
		loopgoto = Players[v]
		repeat
			if Players:FindFirstChild(v) and speaker.Character and Players[v].Character then
				local root = speaker.Character:FindFirstChild("HumanoidRootPart")
				local troot = Players[v].Character:FindFirstChild("HumanoidRootPart")

				if root and troot then
					root.CFrame = troot.CFrame + Vector3.new(distance, 1, 0)
				end
				task.wait(lDelay)
			else
				loopgoto = nil
...
addcmd('headsit',{},function(args, speaker)
	local player = Players[getPlayer(args[1], speaker)[1]]
	...
	headSit = RunService.Heartbeat:Connect(function()
		...
		if hum and hum.RootPart and troot then
			hum.Sit = true
			hum.RootPart.CFrame = troot.CFrame * CFrame.Angles(0, math.rad(0), 0) * CFrame.new(0, 1.6, 0.4)
		else
			headSit:Disconnect()
		end
	end)
...
addcmd('whisper',{'pm'},function(args, speaker)
	-- Aware it's broken for TextChatService, will fix someday
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		task.spawn(function()
			local plrName = Players[v].Name
...
addcmd('spamspeed',{},function(args, speaker)
	...
	if speed then spamspeed = speed end
end)
...
addcmd('creeper',{},function(args, speaker)
	local char = speaker.Character
	if char then
		char[r15(speaker) and "LeftUpperArm" or "Left Arm"]:Destroy()
		char[r15(speaker) and "RightUpperArm" or "Right Arm"]:Destroy()
		char:FindFirstChildOfClass("Humanoid"):RemoveAccessories()
	end
end)

function getTorso(x)
	local x = x or Players.LocalPlayer.Character
...
addcmd("bang", {"rape"}, function(args, speaker)
	...
	bang = animator:LoadAnimation(bangAnim)
	bang:Play(0.1, 1, 1)
	...
	if args[1] then
		...
		for _, v in pairs(players) do
			local bangplr = Players[v]
			bangLoop = RunService.PreSimulation:Connect(function()
				pcall(function()
					local otherRoot = getTorso(bangplr.Character)
					getRoot(speaker.Character).CFrame = otherRoot.CFrame * CFrame.new(0, 0, 1.1)
				end)
			end)
		end
	end
...
addcmd('carpet',{},function(args, speaker)
	...
	if not r15(speaker) then
		...
		for i,v in pairs(players)do
			...
			carpet = animator:LoadAnimation(carpetAnim)
			carpet:Play(.1, 1, 1)
...
addcmd('grabtools', {}, function(args, speaker)
	...
	grabtoolsFunc = workspace.ChildAdded:Connect(grabTool)
	for _, child in ipairs(workspace:GetChildren()) do grabTool(child) end

	if args[1] ~= 'nonotify' then notify("Grabtools", "Picking up any dropped tools") end
end)
...
addcmd('nograbtools',{'ungrabtools'},function(args, speaker)
	...
	if args[1] ~= 'nonotify' then notify("Grabtools", "Grabtools has been disabled") end
end)
...
addcmd('removespecifictool',{},function(args, speaker)
	if args[1] then
		local toolName = string.lower(getstring(1))
		if not toolRemoveCon then
			toolRemoveCon = speaker.Backpack.ChildAdded:Connect(function(tool)
				if tool.Name:lower() == toolName then
					tool:Destroy()
				end
			end)
		end

		table.insert(specifictoolremoval, toolName)
	end
end)

addcmd('unremovespecifictool',{},function(args, speaker)
	if args[1] then
		local tool = table.find(specifictoolremoval, args[1]:lower())
		if tool then
			table.remove(specifictoolremoval, tool)
		end
	end
end)

addcmd('clearremovespecifictool',{},function(args, speaker)
	if toolRemoveCon then
		toolRemoveCon:Disconnect()
		toolRemoveCon = nil
		table.clear(specifictoolremoval)
	end
end)
...
addcmd('light',{},function(args, speaker)
	...
	light.Range = tonumber(args[1]) or 30
	light.Brightness = tonumber(args[2]) or 5
end)
...
addcmd('spawnpoint',{'spawn'},function(args, speaker)
	...
	notify('Spawn Point','Spawn point created at ' .. tostring(spawnpos.Position))
end)
...
addcmd('hatspin',{'spinhats'},function(args, speaker)
	...
	for _,v in pairs(speaker.Character:FindFirstChildOfClass('Humanoid'):GetAccessories()) do
		...
		spin.AngularVelocity = Vector3.new(0, args[1] or 100, 0)
		spin.MaxTorque = Vector3.new(0, (args[1] and args[1] * 2) or 200, 0)
		keep.P = 30000
		keep.D = 50
		spinhats = RunService.PreSimulation:Connect(function()
			local head = speaker.Character and speaker.Character:FindFirstChild("Head")
			if head then
				keep.Position = head.Position
			end
		end)
	end
end)
...
addcmd('split',{},function(args, speaker)
	if rcdEnabled() then notify('Clientsided', 'This command is only visible on your screen due to an update by ROBLOX!') end
	if r15(speaker) then
...
addcmd('dupetools', {'clonetools'}, function(args, speaker)
	if rcdEnabled() then return notify('RejectCharacterDeletions', 'RejectCharacterDeletions is enabled, this command will not work in this game.') end
	...
	for i = 1, LOOP_NUM do
		...
		if i == LOOP_NUM or i % 5 == 0 then
			...
			if vtype(firetouchinterest, "function") then
...
addcmd('touchinterests', {'touchinterest', 'firetouchinterests', 'firetouchinterest'}, function(args, speaker)
	...
	local function touch(x)
		local x = x:FindFirstAncestorWhichIsA("BasePart")
...
addcmd('loopfullbright',{'loopfb'},function(args, speaker)
	...
	brightLoop = RunService.PreRender:Connect(function()
		Lighting.Brightness = 2
		Lighting.ClockTime = 14
		Lighting.FogEnd = 100000
		Lighting.GlobalShadows = false
...
origsettings = {Ambient = Lighting.Ambient, OutdoorAmbient = Lighting.OutdoorAmbient, Brightness = Lighting.Brightness, ClockTime = Lighting.ClockTime, FogEnd = Lighting.FogEnd, FogStart = Lighting.FogStart, GlobalShadows = Lighting.GlobalShadows}
addcmd('restorelighting',{'rlighting'},function(args, speaker)
	for i, v in pairs(origsettings) do Lighting[i] = v end
end)
...
addcmd('reach',{},function(args, speaker)
	...
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
			...
			v.Handle.Size = Vector3.new(0.5,0.5, args[1] or 60)
...
addcmd("antichatlogs", {"antichatlogger"}, function(args, speaker)
	...
	local MessagePosted, _ = pcall(function()
		rawset(require(speaker.PlayerScripts.ChatScript.ChatMain), "MessagePosted", {
			["fire"] = function(msg)
				return msg
			end,
			["wait"] = function()
...
function SkidFling(TargetPlayer)
	...
	if Character and Humanoid and RootPart then
		...
		local SFBasePart = function(BasePart)
			...
			until BasePart.Velocity.Magnitude > 500 or BasePart.Parent ~= TargetPlayer.Character or TargetPlayer.Parent ~= Players or not TargetPlayer.Character == TCharacter or TRootPart:IsGrounded() or Humanoid.Health <= 0 or tick() > Time + TimeToWait
		end
		...
		repeat
			...
			Character:PivotTo(OldPos * CFrame.new(0, .5, 0))
...
addcmd('skidfling', {'sfling'}, function(args, speaker)
	-- Full credit to AnthonyIsntHere
	for _, v in ipairs(getPlayer(args[1], speaker)) do
		local plr = Players[v]
...
addcmd('togglefling',{},function(args, speaker)
	execCmd(flinging and 'unfling' or 'fling')
end)
...
addcmd('tpwalk', {'teleportwalk'}, function(args, speaker)
	...
	while tpwalking and chr and hum and hum.Parent do
		...
		if hum.MoveDirection.Magnitude > 0 then
			if tonumber(args[1]) then
				chr:TranslateBy(hum.MoveDirection * args[1] * delta * 10)
			else
...
addcmd('unspin',{},function(args, speaker)
	local spin = getRoot(speaker.Character) and getRoot(speaker.Character):FindFirstChild("Spinning")
...
addcmd("togglexray", {}, function(args, speaker)
	execCmd(xrayEnabled and 'unxray' or 'xray')
end)

addcmd("loopxray", {}, function(args, speaker)
	execCmd('unloopxray')
	xrayEnabled = true
	xrayLoop = RunService.PreRender:Connect(xray)
end)
...
addcmd('walltp',{},function(args, speaker)
	...
	local function touchedFunc(hit)
		local Root = getRoot(speaker.Character)
		if hit:IsA("BasePart") and humanoid and Root and hit.Position.Y > Root.Position.Y - humanoid.HipHeight then
...
addcmd('autoclick',{},function(args, speaker)
	if mouse1press and mouse1release then
		execCmd('unautoclick')
		task.wait()
		local clickDelay = tonumber(args[1]) and 0.1
		local releaseDelay = tonumber(args[2]) and 0.1
		autoclicking = true
...
addcmd('mousesensitivity',{'ms'},function(args, speaker)
	local GameSettings = cloneref(UserSettings():GetService("UserGameSettings"))

	if tonumber(args[1]) then
		UserInputService.MouseDeltaSensitivity = args[1] / GameSettings.MouseSensitivity
	end
end)
...
addcmd('hovername',{},function(args, speaker)
	...
	local function updateNameBox()
		...
		local t = humanoid and humanoid.Parent
...
addcmd('headsize',{},function(args, speaker)
	...
	local Size = Vector3.new(sizeArg or 1, sizeArg or 1, sizeArg or 1)

	for i,v in pairs(players) do
		local Head = Players[v].Character:FindFirstChild('Head')
		if Players[v] ~= speaker and Head then
			Head.Size = Size
		end
	end
end)
...
addcmd('hitbox',{},function(args, speaker)
	...
	local sizeArg = tonumber(args[2])
	local Size = Vector3.new(sizeArg or 2, sizeArg or 1, sizeArg or 1)

	for i,v in pairs(players) do
		local Root = Players[v].Character and Players[v].Character:FindFirstChild('HumanoidRootPart')

		if Players[v] ~= speaker and Root then
			Root.Size = Size
...
addcmd('stareat',{'stare'},function(args, speaker)
	...
	for i,v in pairs(players) do
		stareLoop = RunService.PreRender:Connect(function()
			local root = speaker.Character and speaker.Character:FindFirstChild("HumanoidRootPart")
			local targetRoot = Players[v].Character and Players[v].Character:FindFirstChild("HumanoidRootPart")
...
addcmd("staffwatch", {}, function(args, speaker)
	...
	if game.CreatorType == Enum.CreatorType.Group then
		...
		notify("Staffwatch", #found > 0 and table.concat(found, ",\n") or "Enabled")
	else
...
addcmd('destroyheight',{'dh'},function(args, speaker)
	...
	if tonumber(dh) then workspace.FallenPartsDestroyHeight = dh end
end)
...
addcmd("antivoid", {}, function(args, speaker)
	...
	task.wait()
	antivoidloop = RunService.PreSimulation:Connect(function()
		local root = getRoot(speaker.Character)
...
addcmd("trip", {}, function(args, speaker)
	...
	if root then
		...
		root.RotVelocity = root.CFrame.LookVector * 30
	end
end)

addcmd("removeads", {"adblock"}, function(args, speaker)
	local function removeAds(v)
	...
	if blockConnect then blockConnect:Disconnect() end
	blockConnect = workspace.DescendantAdded:Connect(removeAds)
	for _, v in pairs(workspace:GetDescendants()) do removeAds(v) end
end)
...
addcmd("listento", {}, function(args, speaker)
	execCmd("unlistento")
	local plr = args[1] and Players:FindFirstChild(getPlayer(args[1], speaker)[1])
	if not plr then return end

	local function setListenTo(char)
		local root = char:WaitForChild("HumanoidRootPart")
		SoundService:SetListener(Enum.ListenerType.ObjectPosition, root)
	end

	if plr.Character then setListenTo(plr.Character) end
	listentoChar = plr.CharacterAdded:Connect(setListenTo)
end)

addcmd("listentopos", {}, function(args, speaker)
	SoundService:SetListener(Enum.ListenerType.CFrame, getPos(args))
end)

addcmd("unlistento", {}, function(args, speaker)
	SoundService:SetListener(Enum.ListenerType.Camera)

	if listentoChar then
...
addcmd('freezeunanchored',{'freezeua'},function(args, speaker)
	...
	local function FREEZENOOB(v)
		if (v:IsA("BasePart") or v:IsA("UnionOperation")) and v.Anchored == false then
			local BADD = false
			for i = 1,#badnames do
...
local partBlacklist = {"Torso", "Head", "Right Arm", "Left Arm", "Right Leg", "Left Leg", "HumanoidRootPart"}
addcmd('tpunanchored',{'tpua'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		local Forces = {}
		for _,part in pairs(workspace:GetDescendants()) do
			if Players[v].Character:FindFirstChild('Head') and (part:IsA("BasePart") or part:IsA("UnionOperation") or part:IsA("Model")) and not part.Anchored and not part:IsDescendantOf(speaker.Character) and not table.find(partBlacklist, part.Name) then
...
addcmd('autokeypress',{'keypress'},function(args, speaker)
	if keypress and keyrelease and args[1] then
		...
		task.wait()
		local clickDelay = tonumber(args[2]) or 0.1
		local releaseDelay = tonumber(args[3]) or 0.1
...
function hookCharEvents(plr,instant)
	task.spawn(function()
		local humanoid = plr.Character and plr.Character:WaitForChild("Humanoid", 10)
		if not humanoid then return end
		...
		humanoid.HealthChanged:Connect(function(health)
			...
			if oldHealth > health then
				eventEditor.FireEvent("OnDamage", plr.Name, health)
			end
			oldHealth = health
		end)
...
Players.PlayerAdded:Connect(function(plr)
	...
	if isLegacyChat then
		plr.Chatted:Connect(function(msg)
			eventEditor.FireEvent("OnChatted", plr.Name, msg)
		end)
	end

	plr.CharacterAdded:Connect(function()
		eventEditor.FireEvent("OnSpawn", plr.Name)
		hookCharEvents(plr)
	end)
	JoinLog(plr)
...
task.spawn(function()
	...
	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify("Outdated", "Use the new version at infyiff.github.io while I update")
		end
