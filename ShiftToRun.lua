...
if run_config.UseCtrlLock then
	if boundKeys then
...
local tweenInfo = TweenInfo.new(run_config.ChangeTime, Enum.EasingStyle.Linear, Enum.EasingDirection.In)
UserInputService.InputBegan:Connect(function(input, processed)
	if input.KeyCode == Enum.KeyCode.LeftShift and not processed then
		local speed = TweenService:Create(humanoid, tweenInfo, {["WalkSpeed"] = run_config.Speed})
		speed:Play()

		if run_config.FovChange == true and workspace.CurrentCamera then
			local tween = TweenService:Create(workspace.CurrentCamera, tweenInfo, {["FieldOfView"] = run_config.Fov})
			tween:Play()
		end
	end
end)
...
UserInputService.InputEnded:Connect(function(input, processed)
	if input.KeyCode == Enum.KeyCode.LeftShift then
		...
		if run_config.FovChange == true and workspace.CurrentCamera and oldFov then
